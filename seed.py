                ...
                for i, track in enumerate(online_targets):
                    if i < len(features):
                        local_id = local_reid_manager.match_or_create(
                            features[i], bboxes[i], camera_id, frame_id
                        )
...
class LocalReIDManager:
    ...
    """
    def __init__(self, similarity_threshold=0.6, feature_ttl=300):
    ...
    def match_or_create(self, features, bbox, camera_id, frame_id):
        ...
        for local_id, track_info in self.local_tracks.items():
            ...
            # 가장 최근 특징과 비교
            latest_feature = track_info['features'][-1]
            similarity = 1 - cdist([features], [latest_feature], 'cosine')[0][0]
        ...
        if best_match_id is not None:
            ...
            # 특징 개수 제한 (메모리 관리)
            if len(self.local_tracks[best_match_id]['features']) > 10:
                self.local_tracks[best_match_id]['features'] = self.local_tracks[best_match_id]['features'][-10:]
            
            print(f"Local ReID: Matched to existing track {best_match_id} (similarity: {best_similarity:.3f})")
            ...
            print(f"Local ReID: Created new track {local_id}")
            return local_id
